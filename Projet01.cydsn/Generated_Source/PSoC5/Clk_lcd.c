...
void Clk_lcd_Start(void) 
{
    ...
    Clk_lcd_CLKEN |= Clk_lcd_CLKEN_MASK;
	Clk_lcd_CLKSTBY |= Clk_lcd_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clk_lcd_Stop
********************************************************************************
*
* Summary:
...
void Clk_lcd_Stop(void) 
{
    ...
    Clk_lcd_CLKEN &= (uint8)(~Clk_lcd_CLKEN_MASK);
	Clk_lcd_CLKSTBY &= (uint8)(~Clk_lcd_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clk_lcd_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clk_lcd_CLKEN & Clk_lcd_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clk_lcd_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clk_lcd_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(Clk_lcd_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clk_lcd_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clk_lcd_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clk_lcd_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clk_lcd_GetSourceRegister();
    uint16 oldDivider = Clk_lcd_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clk_lcd_CLKEN & Clk_lcd_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clk_lcd_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clk_lcd_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clk_lcd_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clk_lcd_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clk_lcd_MOD_SRC |= modeBitMask & (uint8)Clk_lcd_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clk_lcd_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clk_lcd_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clk_lcd_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clk_lcd_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clk_lcd_MOD_SRC & (uint8)(Clk_lcd_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clk_lcd_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clk_lcd_GetDividerRegister();
    uint8 oldSrc = Clk_lcd_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clk_lcd_MOD_SRC |= CYCLK_SSS;
        Clk_lcd_MOD_SRC =
            (Clk_lcd_MOD_SRC & (uint8)(~Clk_lcd_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clk_lcd_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clk_lcd_MOD_SRC & Clk_lcd_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clk_lcd_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clk_lcd_PHASE = clkPhase & Clk_lcd_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clk_lcd_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clk_lcd_PHASE & Clk_lcd_PHASE_MASK;
}
